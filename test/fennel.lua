...
local function _47_(...)
  ...
  local function wrap_env(env)
    ...
    return setmetatable({}, {__newindex = _317_, __pairs = _319_, __index = _315_})
  end
  ...
  local function _84_(ast, scope, parent)
    ...
    compiler.destructure(ast[2], ast[3], ast, scope, parent, {forceglobal = true, symtype = "global", nomulti = true})
    return nil
  end
  ...
  local function _86_(ast, scope, parent)
    compiler.assert((#ast == 3), "expected name and value", ast)
    compiler.destructure(ast[2], ast[3], ast, scope, parent, {isvar = true, symtype = "var", nomulti = true, declaration = true})
    return nil
  end
  SPECIALS.var = _86_
  ...
  local function safe_compiler_env()
    return {pairs = pairs, ipairs = ipairs, getmetatable = safe_getmetatable, setmetatable = setmetatable, rawget = rawget, rawset = rawset, rawequal = rawequal, select = select, tonumber = tonumber, tostring = tostring, error = error, pcall = pcall, xpcall = xpcall, math = utils.copy(math), require = safe_require, assert = assert, string = utils.copy(string), print = print, rawlen = rawget(_G, "rawlen"), bit = rawget(_G, "bit"), table = utils.copy(table), type = type, next = next}
  end
  ...
  local function make_compiler_env(ast, scope, parent, _3fopts)
    ...
    env = {["varg?"] = utils["varg?"], unpack = unpack, ["list?"] = utils["list?"], ["multi-sym?"] = utils["multi-sym?"], view = view, ["sequence?"] = utils["sequence?"], ["macro-loaded"] = macro_loaded, list = utils.list, gensym = _434_, ["table?"] = utils["table?"], macroexpand = _437_, _AST = ast, _CHUNK = parent, _IS_COMPILER = true, _SCOPE = scope, _SPECIALS = compiler.scopes.global.specials, _VARARG = utils.varg(), sym = utils.sym, ["assert-compile"] = compiler.assert, ["sym?"] = utils["sym?"], sequence = utils.sequence, ["get-scope"] = _435_, ["in-scope?"] = _436_}
    env._G = env
  ...
  return {["current-global-names"] = current_global_names, doc = doc_2a, ["macro-searchers"] = macro_searchers, ["make-compiler-env"] = make_compiler_env, ["search-module"] = search_module, ["make-searcher"] = make_searcher, ["macro-loaded"] = macro_loaded, ["wrap-env"] = wrap_env, ["load-code"] = load_code}
end
...
local function _153_(...)
  ...
  local function make_scope(parent)
    ...
    return {includes = setmetatable({}, {__index = (parent0 and parent0.includes)}), gensyms = setmetatable({}, {__index = (parent0 and parent0.gensyms)}), autogensyms = setmetatable({}, {__index = (parent0 and parent0.autogensyms)}), manglings = setmetatable({}, {__index = (parent0 and parent0.manglings)}), macros = setmetatable({}, {__index = (parent0 and parent0.macros)}), unmanglings = setmetatable({}, {__index = (parent0 and parent0.unmanglings)}), parent = parent0, hashfn = (parent0 and parent0.hashfn), specials = setmetatable({}, {__index = (parent0 and parent0.specials)}), depth = _182_, vararg = (parent0 and parent0.vararg), refedglobals = setmetatable({}, {__index = (parent0 and parent0.refedglobals)}), symmeta = setmetatable({}, {__index = (parent0 and parent0.symmeta)})}
  end
  ...
  local function make_metadata()
    ...
    return setmetatable({}, {__mode = "k", __index = {get = _223_, setall = _226_, set = _225_}})
  end
  ...
  return {scopes = scopes, compile1 = compile1, destructure = destructure, ["global-unmangling"] = global_unmangling, ["apply-manglings"] = apply_manglings, ["declare-local"] = declare_local, ["symbol-to-expression"] = symbol_to_expression, metadata = make_metadata(), emit = emit, macroexpand = macroexpand_2a, ["keep-side-effects"] = keep_side_effects, compile = compile, ["do-quote"] = do_quote, assert = assert_compile, gensym = gensym, autogensym = autogensym, ["require-include"] = require_include, traceback = traceback, ["compile-string"] = compile_string, ["compile-stream"] = compile_stream, ["make-scope"] = make_scope, ["global-mangling"] = global_mangling}
end
...
local function _258_(...)
  ...
  local suggestions = {["unexpected multi symbol (.*)"] = {"removing periods or colons from %s"}, ["use of global (.*) is aliased by a local"] = {"renaming local %s", "refer to the global using _G.%s instead of directly"}, ["expected even number of name/value bindings"] = {"finding where the identifier or value is missing"}, ["could not compile value of type "] = {"debugging the macro you're calling to return a list or table"}, ["expected macros to be table"] = {"ensuring your macro definitions return a table"}, ["expected each macro to be function"] = {"ensuring that the value for each key in your macros table contains a function", "avoid defining nested macro tables"}, ["expected var (.*)"] = {"declaring %s using var instead of let/local", "introducing a new local instead of changing the value of %s"}, ["malformed multisym"] = {"ensuring each period or colon is not followed by another period or colon"}, ["method must be last component"] = {"using a period instead of a colon for field access", "removing segments after the colon", "making the method call, then looking up the field on the result"}, ["tried to reference a macro at runtime"] = {"renaming the macro so as not to conflict with locals"}, ["macro not found in macro module"] = {"checking the keys of the imported macro module's returned table"}, ["macro tried to bind (.*) without gensym"] = {"changing to %s# when introducing identifiers inside macros"}, ["unknown identifier in strict mode: (.*)"] = {"looking to see if there's a typo", "using the _G table instead, eg. _G.%s if you really want a global", "moving this code to somewhere that %s is in scope", "binding %s as a local in the scope of this code"}, ["unexpected iterator clause"] = {"removing an argument", "checking for typos"}, ["expected a function.* to call"] = {"removing the empty parentheses", "using square brackets if you want an empty table"}, ["cannot call literal value"] = {"checking for typos", "checking for a missing function name"}, ["expected binding and iterator"] = {"making sure you haven't omitted a local name or iterator"}, ["unused local (.*)"] = {"renaming the local to _%s if it is meant to be unused", "fixing a typo so %s is used", "disabling the linter which checks for unused locals"}, ["expected parameters"] = {"adding function parameters as a list of identifiers in brackets"}, ["unable to bind (.*)"] = {"replacing the %s with an identifier"}, ["expected symbol for function parameter: (.*)"] = {"changing %s to an identifier instead of a literal value"}, ["unexpected vararg"] = {"putting \"...\" at the end of the fn parameters if the vararg was intended"}, ["multisym method calls may only be in call position"] = {"using a period instead of a colon to reference a table's fields", "putting parens around this"}, ["expected rest argument before last parameter"] = {"moving & to right before the final identifier when destructuring"}, ["unexpected arguments"] = {"removing an argument", "checking for typos"}, ["expected even number of pattern/body pairs"] = {"checking that every pattern has a body to go with it", "adding _ before the final body"}, ["$ and $... in hashfn are mutually exclusive"] = {"modifying the hashfn so it only contains $... or $, $1, $2, $3, etc"}, ["can't start multisym segment with a digit"] = {"removing the digit", "adding a non-digit before the digit"}, ["could not read number (.*)"] = {"removing the non-digit character", "beginning the identifier with a non-digit if it is not meant to be a number"}, ["may only be used at compile time"] = {"moving this to inside a macro if you need to manipulate symbols/lists", "using square brackets instead of parens to construct a table"}, ["illegal character: (.)"] = {"deleting or replacing %s", "avoiding reserved characters like \", \\, ', ~, ;, @, `, and comma"}, ["local (.*) was overshadowed by a special form or macro"] = {"renaming local %s"}, ["unexpected closing delimiter (.)"] = {"deleting %s", "adding matching opening delimiter earlier"}, ["expected vararg as last parameter"] = {"moving the \"...\" to the end of the parameter list"}, ["expected local"] = {"looking for a typo", "looking for a local which is used out of its scope"}, ["mismatched closing delimiter (.), expected (.)"] = {"replacing %s with %s", "deleting %s", "adding matching opening delimiter earlier"}, ["expected body expression"] = {"putting some code in the body of this form after the bindings"}, ["expected even number of values in table literal"] = {"removing a key", "adding a value"}, ["expected binding sequence"] = {"placing a table here in square brackets containing identifiers to bind"}, ["global (.*) conflicts with local"] = {"renaming local %s"}, ["expected whitespace before opening delimiter"] = {"adding whitespace"}}
  local unpack = (table.unpack or _G.unpack)
...
local function _268_(...)
  ...
  local function parser(getbyte, filename, options)
    ...
    local function parse_stream()
      ...
      local function open_table(b)
        ...
        return table.insert(stack, {filename = filename, line = line, closer = delims[b], bytestart = byteindex})
      end
      ...
      local function parse_sym(b)
        ...
        elseif not parse_number(rawstr) then
          return dispatch(utils.sym(check_malformed_sym(rawstr), {filename = filename, byteend = byteindex, line = line, bytestart = bytestart}))
        else
          return nil
        end
      end
  ...
  return {parser = parser, ["sym-char?"] = sym_char_3f, granulate = granulate, ["string-stream"] = string_stream}
end
package.preload["fennel.parser"] = (package.preload["fennel.parser"] or _268_)
local utils = nil
local function _305_(...)
  local type_order = {thread = 7, ["function"] = 5, userdata = 6, boolean = 2, number = 1, string = 3, table = 4}
  ...
  local function pp_string(str, options, indent)
    ...
    escs = setmetatable({["\13"] = "\\r", ["\8"] = "\\b", ["\\"] = "\\\\", ["\12"] = "\\f", ["\9"] = "\\t", ["\n"] = _71_, ["\""] = "\\\"", ["\11"] = "\\v", ["\7"] = "\\a"}, {__index = _73_})
    return ("\"" .. str:gsub("[%c\\\"]", escs) .. "\"")
  end
  local function make_options(t, options)
    local defaults = {["empty-as-sequence?"] = false, ["prefer-colon?"] = false, ["max-sparse-gap"] = 10, ["one-line?"] = false, ["detect-cycles?"] = true, ["utf8?"] = true, ["escape-newlines?"] = false, ["line-length"] = 80, ["metamethod?"] = true, depth = 128}
...
local function _350_(...)
  ...
  local symbol_mt = {"SYMBOL", __lt = sym_3c, __tostring = deref, __fennelview = deref, __eq = sym_3d}
  local expr_mt = nil
  ...
  local comment_mt = {"COMMENT", __lt = sym_3c, __tostring = deref, __fennelview = comment_view, __eq = sym_3d}
  ...
  root = {options = nil, scope = nil, reset = _112_, chunk = nil}
  ...
  return {["quoted?"] = quoted_3f, ["varg?"] = varg_3f, ["macro-path"] = table.concat({"./?.fnl", "./?/init-macros.fnl", "./?/init.fnl", getenv("FENNEL_MACRO_PATH")}, ";"), ["valid-lua-identifier?"] = valid_lua_identifier_3f, ["propagate-options"] = propagate_options, varg = varg, allpairs = allpairs, root = root, path = table.concat({"./?.fnl", "./?/init.fnl", getenv("FENNEL_PATH")}, ";"), stablepairs = stablepairs, ["list?"] = list_3f, ["walk-tree"] = walk_tree, kvmap = kvmap, ["sequence?"] = sequence_3f, hook = hook, ["lua-keywords"] = lua_keywords, ["sym?"] = sym_3f, ["table?"] = table_3f, ["multi-sym?"] = multi_sym_3f, sym = sym, ["debug-on?"] = debug_on_3f, comment = comment_2a, ["member?"] = member_3f, ["comment?"] = comment_3f, expr = expr, ["expr?"] = expr_3f, warn = warn, list = list, sequence = sequence, copy = copy, map = map}
end
...
local function syntax()
  ...
  for k, v in pairs(compiler.scopes.global.specials) do
    ...
    out[k] = {["binding-form?"] = utils["member?"](k, binding_3f), ["body-form?"] = metadata["fnl/body-form?"], ["special?"] = true, ["define?"] = utils["member?"](k, define_3f)}
  end
...
local mod = {varg = utils.varg, parser = parser.parser, path = utils.path, view = view, ["sequence?"] = utils["sequence?"], mangle = compiler["global-mangling"], unmangle = compiler["global-unmangling"], searcher = specials["make-searcher"](), ["macro-searchers"] = specials["macro-searchers"], loadCode = specials["load-code"], ["search-module"] = specials["search-module"], ["make-searcher"] = specials["make-searcher"], granulate = parser.granulate, ["string-stream"] = parser["string-stream"], compileString = compiler["compile-string"], stringStream = parser["string-stream"], metadata = compiler.metadata, makeSearcher = specials["make-searcher"], compile1 = compiler.compile1, compileStream = compiler["compile-stream"], macroLoaded = specials["macro-loaded"], searchModule = specials["search-module"], make_searcher = specials["make-searcher"], syntax = syntax, ["macro-path"] = utils["macro-path"], ["list?"] = utils["list?"], repl = repl, version = "1.0.0-dev", dofile = dofile_2a, ["load-code"] = specials["load-code"], eval = eval, sequence = utils.sequence, compile = compiler.compile, doc = specials.doc, list = utils.list, scope = compiler["make-scope"], ["comment?"] = utils["comment?"], sym = utils.sym, gensym = compiler.gensym, comment = utils.comment, ["macro-loaded"] = specials["macro-loaded"], traceback = compiler.traceback, ["sym?"] = utils["sym?"], ["compile-stream"] = compiler["compile-stream"], ["compile-string"] = compiler["compile-string"], ["sym-char?"] = parser["sym-char?"]}
...
do
  ...
  local built_ins = eval(builtin_macros, {allowedGlobals = false, env = env, moduleName = module_name, scope = compiler.scopes.compiler, filename = "src/fennel/macros.fnl", useMetadata = true})
